...
class AddBus(models.Model):
    ...
    seat_type = models.ManyToManyField(
        BusServiceRegistration, through='SeatInfo')
...
class SeatInfo(models.Model):
    bus_id = models.ForeignKey(AddBus, on_delete=models.CASCADE)
    bus_details = models.ForeignKey(
        BusServiceRegistration,
        on_delete=models.CASCADE)
    seat_type = models.CharField(
        max_length=100, choices=[
            ('Luxury', 'Luxury'), ('Ordinary', 'Ordinary'), ('Sleeper', 'Sleeper')])
    ...
    def __str__(self):
        return self.bus_id.bus_name + ' ' + \
            self.seat_type + ' ' + str(self.seat_no)
